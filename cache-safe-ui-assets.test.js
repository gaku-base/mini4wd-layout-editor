...
const LEGACY_ENTRY_CACHE = 'v1.1-rc4-20260820-toolbar-trash1';
const HEALTH_CACHE = 'v1.1-rc6-health1';

test('critical overlap fix stays static while extension assets share one current cache key', () => {
  ...
  // index.html still owns the two long-lived entry assets. They stay explicitly
  // cache-busted until the HTML shell is cleaned in the next maintenance stage.
  assert.match(index, new RegExp(`styles\\.css\\?v=${LEGACY_ENTRY_CACHE}`));
  assert.match(index, new RegExp(`wheel-rotation\\.js\\?v=${LEGACY_ENTRY_CACHE}`));
