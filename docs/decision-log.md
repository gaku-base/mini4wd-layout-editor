...
### ストレートの赤・青カラーによるSlope変換モード（2026-09-18）

- カラー変更時のストレート挙動を、右側の選択パネルで2択にする。
  - `デフォルト（赤=上り / 青=下り）`
  - `カラーのみ（パーツ変更なし）`
- デフォルトでは、**ストレート**を赤へ変更するとSlopeへ変換し、Startの出口からたどった進行方向で低端→高端となる「上り」にする。青へ変更すると高端→低端となる「下り」にする。
- 「カラーのみ」では赤・青を含め、従来どおり `colorKey` だけを変更し、パーツ種別・向き・高さを変更しない。
- 進行方向は画面上の左右やパーツ回転角から推測せず、Startの`exit`コネクタを起点に既存接続edgeをたどって対象パーツへ到達する側を入口と判定する。Startへつながっていない、または方向が一意でない場合はSlope変換を行わない。
- Straight→Slope変換時は、現在の物理接続位置を動かさないよう、必要に応じて180°回転し、接続edgeのコネクタIDを同時に対応付け直す。
- 高さはStart出口を0mm基準として、接続edgeと各コネクタの`localZMm`から再計算する。赤の上りでは後続コースを+115mm、青の下りでは後続コースを-115mm方向へ整合させる。
- 青の下り変換でコースが0mm未満になる場合はfail-closedで変更しない。
- 閉ループで上りだけ先に設定した場合は、Start入口へ戻る地点の高さ不一致を残す。後から下りを設定して整合する編集を許可するため、途中状態だけを理由に上り変換そのものを取消さない。
- 保存済みJSON/localStorageのレイアウト形式は変更しない。モード選択は実行中のUI設定で、再起動時はデフォルトへ戻る。
- 追加の保存メタデータを持たないため、通常配置したSlopeを最初に赤/青へ色変更しただけでは向きを変更しない。一方、すでに赤または青のSlopeをもう一方の色へ変更した場合は、カラー操作で生成されたSlopeと同様にStart方向へ再整列できる。この制約は保存形式を維持するための最小実装とする。
- 変換が拒否された場合はUndo履歴を消費しない。
- 寸法は既存の`PART_DIMENSIONS_MM`をそのまま使用し、Slope高低差115mmを変更しない。


### 全コースパーツのレイアウト占有寸法監査・採用方針（2026-09-16）

- タミヤ公式の寸法表・競技規則・製品ページは**公称／参考寸法**として保持する。製品の成形・ジョイント・実組立時の差をすべて含む配置寸法とは扱わない。
- レイアウトスペースへ配置する実用寸法は、2026-09-16時点でアプリが使用している寸法を、成形誤差を考慮した**プロジェクト採用寸法（molded-fit dimensions）**として正式採用する。
- 45° Cornerは現行の中心R540mm・内R360mm・外R720mmを採用する。タミヤ公式の90°集成717mm／180°集成1434mmは参考値として残すが、配置ジオメトリを717mmへ補正しない。
- Slopeは現行の水平540mm・高低差115mmを採用する。タミヤ製品説明の「落差11cm」は公称参考値であり、115mmを110mmへ変更しない。
- Straight／Start 540×360mm、Lane Change 1620×360mm、Wave 540×420mm、Bank20 230×360mm、LC Jump 540×360mm、Burning Lane Change 1800×1440mm等、現行runtimeが持つ各寸法を採用値として扱う。
- 今後の実測・現物合わせで寸法を修正できるよう、`part-catalog.js` 内にmm単位の `PART_DIMENSIONS_MM` を唯一の寸法マスターとして置き、PARTSの配置・接続・表示外形をそこから生成する。
- 現在のCanvas/world XYは後方互換のためcm表現を維持するが、寸法マスターからカタログを生成するときだけmm→cm変換する。保存形式は変更しない。
- 描画側も、パーツ固有の `def.w / def.h / geometry` を参照する。将来特定パーツだけ寸法を変えた場合に、360mm等の旧固定値が描画へ残らないようにする。
- 公式値と採用値は混同しない。公式値は `OFFICIAL_DIMENSION_REFERENCES_MM` に分離し、採用ジオメトリを自動上書きしない。
- 将来の物理採寸は「現在寸法が未確定だから必須」ではなく、採用寸法をさらに現物へ合わせる**任意の校正作業**として扱う。Issue #117で追跡する。

### 新規レイアウト作成モーダルのスペース設定タブ（2026-08-04）

- 「レイアウトスペース」「スペース修正」「干渉物設定」は通常編集画面のモードではなく、
  新規レイアウト作成モーダル内の一時的なタブUIとする。タブ選択はJSON、localStorage、
  Undo/Redoへ保存しない。
- レイアウトスペースは従来の作成サイズ入力、プリセット、Start配置フローをそのまま
  利用する。スペース修正は、既存の `roomCutouts` と部屋形状編集を再実装せず、その導線
  としてのみ扱う。
- 干渉物設定は画面構成と準備中表示だけを提供する。障害物データ、配置、衝突、保存、
  JSON形式は今回追加しない。
...
### Fast path stable selection frame (2026-07-31)

- A single placement cycle captures `selectionFrameHeading` from `activePlacementAnchor.heading`. Ghost type changes may alter the next part's visual exit, but must not alter the left/right selection frame or the free-release frame.
- `selectionPointerOrigin` is set only when a cycle starts (placement confirmation, Start activation, or snap reactivation). It remains fixed while Straight, Right Corner, and Left Corner are previewed, so a type change cannot move the virtual selection coordinate.
- The dashed guide uses the same fixed frame. Only confirmation of a new part or reactivation from an open connector begins a new frame using that next active anchor's heading.

### Room CAD render stability, dimension labels, and effective-room corner snapping (2026-07-31)
