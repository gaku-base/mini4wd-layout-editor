...
- 吸着半径は画面上24pxとし、ワールド距離へズーム率を掛けて判定する。範囲外は自由配置とし、重なりや使用済み接続口を配置拒否理由にしない。Altは押下中のみ吸着を無効化し、タッチ用の吸着切替はレイアウト保存対象外とする。
...
- Altによる一時吸着OFFはキーボード状態に加え、PointerEventの`altKey`も参照する。タッチ用吸着ON／OFFと同じく保存対象には含めない。
