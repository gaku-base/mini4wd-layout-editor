...
### 全コースパーツのレイアウト占有寸法監査（2026-09-16）

- タミヤ公式の寸法表・競技規則・製品ページを、現行 `part-catalog.js` の配置・描画・接続寸法と照合した。監査の詳細と根拠URLは `docs/part-dimension-audit-2026-09-16.md` を正本とする。
- 配置に直接使う寸法のうち、Straight 540mm、Lane Change 1620mm、Slope水平540mm、Slope高低差115mm、Bank20接続口間230mm、Bank20角度20°は現行runtimeと一致しているため変更しない。
- タミヤ公式資料で確認できるJCJC 90°カーブ外形717mm／180°外形幅1434mmに対し、現行45°コーナーのローカルモデルは外半径720mmで3mm差がある。ただし公式資料は中心半径・内半径・外半径・コネクタ位置の内訳を示していないため、集成外形だけからローカル寸法を逆算せず、現行形状を `provisional` のまま維持する。
- タミヤSlope製品説明の「落差11cm」は公開された公称値として別記録する。プロジェクトで承認済みの115mm高低差と詳細縦断形状を置き換えない。
- タミヤ競技規則の1レーン幅115mm（3レーン合計345mm）と、プロジェクト承認済み接続面外幅370mmは別の幾何量である。どちらからもパーツ本体の完全な平面外形を一意に決められないため、従来の360mm表示／占有幅を一律345mmまたは370mmへ変更しない。
- Wave最大外形、LC Jump単体外形、Burning Lane Change実物外形、各パーツの側壁・支持部を含む完全外形は、寸法入りの根拠が不足しているため `unknown` または `provisional` を維持する。未確認値を推測で補完しない。
- 今回は監査メタデータと回帰テストを追加するだけで、配置座標、接続、当たり判定、保存形式、PNG／発表出力の形状は変更しない。
