...
### ストレートの赤・青カラーをスロープ変換コマンドとして扱う選択機能（2026-09-18）

- カラー変更には2つの動作モードを設ける。初期値は `slope-by-color`（UI表記「デフォルト（赤=上り／青=下り）」）、もう一つは `color-only`（UI表記「カラーでのパーツ変更なし」）とする。
- デフォルトでは**ストレートだけ**を特別扱いし、赤を指定するとStartからの進行方向で上りスロープ、青を指定するとStartからの進行方向で下りスロープへ変換する。他パーツの赤・青は通常のカラー変更とする。
- 進行方向は画面の左右や配置順では決めず、Startの `exit` コネクタから明示的な接続edgeをたどって対象ストレートへ入るコネクタを特定する。
- 上り変換では下流接続コンポーネント全体を+115mm、下り変換では-115mmして既存接続の高さを維持する。下りで0mm未満になる場合、Startへ下流が再接続する場合、進行方向が不明／曖昧な場合、バンク状態のストレートはfail-closedで変換しない。
- 赤／青は**変換コマンド**として扱うため、スロープへ変換後の `colorKey` は `default` とする。スロープ自体を赤／青へ塗る操作は、その後の通常カラー変更として行える。
- `color-only` では従来どおりパーツ種類を変更せず、赤ストレート／青ストレートとして色だけ変更する。
- デフォルトモードで「赤を経由せず青を直接選択」できる必要があるため、カラーUIは順送りだけに依存せず、色パレットから直接色を選択できる方式にする。
- 動作モードはレイアウト固有情報ではなくアプリ操作設定なので、レイアウトJSON schemaへ追加せずlocalStorageの専用キーで保持する。既存保存レイアウトとの互換性を維持する。
- ストレート→スロープ変換はXY配置と接続点の実位置を維持し、必要な場合だけ180°回転とconnector IDの対応付けを行う。既存edgeを削除して推測再接続しない。
